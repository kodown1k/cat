...
void UStatComponent::BeginPlay()
{
    ...
    // Uzyskaj referencj� do postaci (actor) - zak�adaj�c, �e komponent jest do��czony do postaci
...
void UStatComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
    ...
    if (MyWidget && (!HealthBar && !EnergyBar && !ExpBar))
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("Nie uda�o si� znale�� HealthBar!"));
        }
...
{
    AcattestCharacter* CharacterOwner = Cast<AcattestCharacter>(GetOwner()); // Pobieramy w�a�ciciela komponentu (Actor)
    if (CharacterOwner->GetIsCrouching()) {
        return;
    }
    // Sprawdzenie, czy w�a�ciciel komponentu jest prawid�owy
    if (!CharacterOwner)
    {
        UE_LOG(LogTemp, Error, TEXT("Owner is not a valid ACharacter!"));
        return; // Zatrzymanie funkcji, je�li w�a�ciciel nie jest poprawny
    }

    if (!bIsSprinting) // Sprawdzenie, czy posta� nie jest ju� w trybie sprintu
    {
        bIsSprinting = true;
        ...
        // Zmie� skal� postaci (je�li chcesz, np. zmniejszaj�c j� podczas sprintu)

        UE_LOG(LogTemp, Log, TEXT("%s is now sprinting"), *GetName());
    }
    else if (bIsSprinting) // Sprawdzenie, czy posta� jest w trybie sprintu
    {
        bIsSprinting = false;
        ...
        // Przywr�� normaln� skal� postaci (je�li zmienia�a si� wcze�niej)
...
void UStatComponent::GetDamaged(int DamageAmount)
{
    ...
    if (m_currentHealth <= 0 && !bIsDead)
    {
        ...
        // Pobierz w�a�ciciela tego komponentu
        ...
        if (Owner)
        {
            // Ustaw now� lokalizacj� w�a�ciciela
...
void UStatComponent::GetRegen()
{
    ...
    if (bIsSprinting) {
        ...
        if (m_currentEnergy <= 0) {
            m_currentEnergy = 0; 
            ACharacter* CharacterOwner = Cast<ACharacter>(GetOwner()); // Pobieramy w�a�ciciela komponentu (Actor)
            bIsSprinting = false;
            CharacterOwner->GetCharacterMovement()->MaxWalkSpeed = m_baseSpeed;
        }
    }
